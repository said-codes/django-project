...
phone_regex = RegexValidator(
    regex=r'^\+?1?\d{1,10}$',
    message="El número de teléfono debe ingresarse en el formato: '+999999999'. Hasta 10 dígitos permitidos."
)
"""
...
class User(AbstractBaseUser, PermissionsMixin):
    ...
    document = models.CharField(
        verbose_name='Documento', max_length=30, blank=True, null=True)
    email = models.EmailField(
        verbose_name='Correo Electrónico', unique=True, max_length=255)
    phone = PhoneField(blank=True, help_text='Teléfono/Celular')
    avatar = models.ImageField(
        verbose_name='Foto Perfil', upload_to='user_profile/', blank=True, null=True)
    is_admin = models.BooleanField(verbose_name='Admin', default=False)
    is_active = models.BooleanField(
        verbose_name='Activo/Inactivo', default=True)
    is_staff = models.BooleanField(verbose_name='Permiso al PA', default=False)
    # Nuevo campo para habilitar/deshabilitar
    enabled = models.BooleanField(default=True)
    groups = models.ForeignKey(Group, on_delete=models.SET_NULL, null=True, verbose_name="Grupo", help_text=(
    ...
    user_permissions = models.ManyToManyField(
        Permission,
        # Nombre único para evitar el choque
        related_name='custom_user_set_permissions',
        blank=True,
        help_text='Specific permissions for this user.',
        verbose_name='Permiso de Usuario',
    )
    created = models.DateTimeField(verbose_name='Creada el', auto_now_add=True,
                                   blank=True, null=True, help_text='La fecha de creacion de este objeto')
    modified = models.DateTimeField(verbose_name='Modificada el', auto_now=True,
                                    blank=True, null=True, help_text='la fecha de modificacion de este objeto')
...
class UtilsData(models.Model):
    ...
    user = models.ForeignKey(User, verbose_name="Creado por",
                             on_delete=models.SET_NULL, blank=True, null=True)


class Customer(UtilsData):
    name = models.CharField(
        verbose_name="Nombre / Razon social", max_length=255)
